...
const Sidebar = ({ expand, setExpand }) => {
  ...
  return (
    <div className={`flex flex-col justify-between bg-[#212327] pt-7 transition-all z-50 max-md:absolute max-md:h-screen ${expand ? 'p-4 w-64' : 'md:w-20 w-0 max-md:overflow-hidden'}`}>
      <div>
        ...
        <button className={`mt-8 flex items-center justify-center cursor-pointer ${expand ? "bg-primary hover:opcity-90 rounded-2xl gap-2 p-2.5 w-max":"group relative h-9 w-9 mx-auto hover:bg-gray-500/30 rounded-lg"}`}>
            <Image className={expand ? 'w-6': 'w-7'} src={expand ? assets.chat_icon : assets.chat_icon_dull} alt='' />
            <div className='absolute w-max -top-12 -right-12 opacity-0 group-hover:opacity-100 transition bg-black text-white text-sm px-3 py-2 rounded-lg shadow-lg pointer-eventes-none'>
                New Chat 
  ...
  <div className={`absolute z-10 pb-8 ${!expand ? "-right-40" : "right-1/2 translate-x-1/2"} -top-60 opacity-0 group-hover:opacity-100 group-hover:block transition hidden`}>
    <div className="relative w-max bg-black text-white text-sm p-3 rounded-lg shadow-lg">
      ...
      <div
        className={`w-3 h-3 absolute bg-black rotate-45 ${expand ? "right-1/2 translate-x-1/2" : "left-4"} -bottom-1.5`}></div>
    </div>
  </div>

  {expand && (<> <span>Get App</span> <Image src={assets.new_icon} alt="new" />
  </>)}
</div>
